...
### Cheap-model playtest results: Sonnet vs Haiku vs DeepSeek

Measured comparison (2026-06-09): a full 10-year homogeneous self-play game per
model (one model drives all seven powers). Generated by
...
the Haiku and DeepSeek sides are research runs kept in gitignored `scratch/` and
...
- **Cost and speed:** three tiers, Sonnet $25.62, Haiku $8.05, DeepSeek $1.17.
  Haiku is the fastest (21.8 min); DeepSeek is the cheapest but slowest (54.5
  min: no gateway prompt caching, higher per-call latency, empty-content
  retries).
- **Competence (trustworthy):** Sonnet is clearly strongest (lowest illegal
  rate, twice the coordination via Support). Among the cheap models DeepSeek
  beats Haiku: lower illegal rate (7.0 vs 11.0%) and far fewer dropped
  unit-orders (0.7 vs 4.4%), at one-seventh the cost.
- **Board (color):** higher N_eff comes from a quieter board, and Haiku is the
  extreme: the highest N_eff (6.45) but by far the lowest Land turnover (7 vs
  Sonnet's 27) and almost no betrayals (5 vs 47), the most static, least
  contested game.
- **Verdict:** DeepSeek V4-Flash is the best low-cost option, undercutting Haiku
  on both cost and mechanical competence; Haiku's only edges are speed and being
  a direct-Anthropic model with no gateway empty-content risk. Kimi K2.6 and
  Gemini 3.5-Flash both run about $20 per 10-year game on this input-heavy,
  uncached workload (as costly as Sonnet), so neither is low-cost here.
