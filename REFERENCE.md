...
### Cheap-model playtest results: Sonnet vs DeepSeek

First measured A/B (2026-06-09), scaled up from the one-year plan to a full
10-year homogeneous self-play game per model (one model drives all seven
powers). Generated by [`reference/compare_models.py`](reference/compare_models.py);
the order and message metrics mirror the dashboard, while N_eff and dropped-turns
are reference-only. Game-level rows are n=1, so rank on the Competence block
(each rate averages over hundreds of orders) and read Board and Negotiation as
color. Sonnet is the committed canonical run
(`results/canonical/2026-06-04.14.48.20`); the DeepSeek side is a research run
kept in gitignored `scratch/` and likely to be deleted once the cheap-model
question is settled, so the table below is the preserved result.
...
- **Cost:** DeepSeek runs at roughly 1/22 the dollar cost but about 1.6x the
  wall-clock (no gateway prompt caching, higher per-call latency, empty-content
  retries).
...
- **Verdict:** DeepSeek V4-Flash is the viable low-cost alternative. Kimi K2.6
  and Gemini 3.5-Flash both run about $20 per 10-year game on this input-heavy,
  uncached workload (as costly as Sonnet), so neither is low-cost here. DeepSeek
  plays a coherent but weaker, more passive game for a fraction of the cost.
